...
def test_system_monitor_initialization(system_monitor):
    ...
    expected_metrics = ["system_cpu_usage", "system_memory_usage", "system_disk_usage"]
...
def test_metrics_update(system_monitor):
    ...
    collected_metrics = {
        metric.name: metric for metric in system_monitor.registry.collect()
    }
...
def test_continuous_monitoring(system_monitor):
    ...
    for _ in range(3):
        system_monitor.update_metrics()
        metrics = {metric.name: metric for metric in system_monitor.registry.collect()}
        readings.append(metrics)
        time.sleep(1)

    # Verify that we get different readings
    cpu_values = [reading["system_cpu_usage"].samples[0].value for reading in readings]
